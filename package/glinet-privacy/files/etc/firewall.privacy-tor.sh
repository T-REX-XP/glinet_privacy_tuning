...
LAN_DEV="$(uci -q get glinet_privacy.firewall.lan_dev 2>/dev/null || true)"
[ -n "$LAN_DEV" ] || LAN_DEV="$(uci -q get network.lan.device 2>/dev/null || echo br-lan)"

ROUTER_LAN_IP="$(uci -q get glinet_privacy.firewall.router_lan_ip 2>/dev/null || true)"
[ -n "$ROUTER_LAN_IP" ] || ROUTER_LAN_IP="$(uci -q get network.lan.ipaddr 2>/dev/null || echo 192.168.8.1)"

LAN_CIDR="$(uci -q get glinet_privacy.firewall.lan_cidr 2>/dev/null || echo 192.168.8.0/24)"
TOR_TRANS_PORT="$(uci -q get glinet_privacy.firewall.tor_trans_port 2>/dev/null || echo 9040)"
TOR_DNS_PORT="$(uci -q get glinet_privacy.firewall.tor_dns_port 2>/dev/null || echo 9053)"
