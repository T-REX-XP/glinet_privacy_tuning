...
**Scope:** `luci-app-glinet-privacy` **JS** views (`htdocs/luci-static/resources/view/glinet_privacy/*.js`), **`luci-json.sh`** (status JSON), **`glinet-privacy`** shell probes **`killswitch-drop-active.sh`** / **`tor-transparent-nat-active.sh`**, **`root/usr/share/luci/menu.d/luci-app-glinet-privacy.json`**, `install.sh`, rpcd ACL (**`file.exec`** for **`luci-json.sh`**).  
...
| **LuCI UI** | **JavaScript** views (`resources/view/glinet_privacy/*.js`) + **`menu.d`** **`type: view`** | **Target: GL.iNet stock only** (`install.sh` enforces). Backend JSON: **`luci-json.sh`** + **`fs.exec`** (ACL **`file.exec`**). No Lua controllers, no **`luci-lua-runtime`**. |
...
*Document version: 2026-04-08 — aligned with **`docs/backlog.md`** and `GLINET_PRIVACY_VERSION` **1.3.0** (`package/version.mk`). Re-check **`changes.md`** on each release.*
