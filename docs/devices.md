...
**Scope:** GL.iNet routers with stock firmware only (`install.sh` enforces this). The same packages and `install.sh` apply across supported models; **WAN defaults** depend on hardware.
...
| `generic` | Unlisted GL.iNet boards | varies | empty |
